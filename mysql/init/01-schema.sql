/*
 * 本文件由 docker-entrypoint-initdb.d 机制在 MySQL 容器
 * 首次启动时自动执行，执行用户为 root。
 */
-- ============================================================
...
--   READY     → 处理完成，可以参与对话检索
--   FAILED    → 处理失败，错误信息记录在 async_task 表
-- ------------------------------------------------------------
...
--   context_recall    = 上下文召回率：标准答案是否被检索覆盖（0~1，越高越好）
